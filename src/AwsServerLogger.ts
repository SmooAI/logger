...
export default class AwsServerLogger extends Logger {
  ...
  private getCaller(level: number): CallerContext {
    ...
    const wrappedFrames: CallSite[] = frames.map((frame) =>
      sourceMapSupport.wrapCallSite(frame as any),
    );
          ...
          [ContextKey.Http]: {
            [ContextKeyHttp.Request]: {
              ...
              [ContextKeyHttpRequest.UserAgent]:
                event?.requestContext?.http?.userAgent || event?.headers?.[ContextHeader.UserAgent],
  ...
  private parseSQSMessageAttributes(messageAttributes: SQSBatchMessageAttributes) {
    return Object.entries(messageAttributes).reduce((result, attribute) => {
      let value: any;
  ...
  public addSQSRecordContext(record: SQSRecord) {
    this.addBaseContext({
      [ContextKey.Queue]: {
        ...
        [ContextKeyQueue.MessageApproximateReceiveCount]:
          record?.attributes?.ApproximateReceiveCount,
      },
      ...this.parseSQSMessageAttributes(
        this.sqsRecordMessageAttributesToBatchMessageAttributes(record?.messageAttributes),
      ),
    });
  }

  protected sqsRecordMessageAttributesToBatchMessageAttributes(
    messageAttributes: SQSRecord["messageAttributes"],
  ): SQSBatchMessageAttributes {
    return Object.entries(messageAttributes).reduce((result, [key, sqsValue]) => {
      ...
      value.BinaryValue = sqsValue.binaryValue
        ? Uint8Array.from(Buffer.from(sqsValue.binaryValue))
        : undefined;
      value.DataType = sqsValue.dataType;
      value.StringListValues = sqsValue.stringListValues;
      value.BinaryListValues = sqsValue.binaryListValues
        ? sqsValue.binaryListValues.map((value) => Uint8Array.from(Buffer.from(value)))
        : undefined;
          ...
          ? JSON.stringify({
              [ContextKeyHttp.Request]: {
                ...
                [ContextKeyHttpRequest.Headers]: {
                  origin:
                    httpContext[ContextKeyHttp.Request][ContextKeyHttpRequest.Headers]?.origin,
                  referer:
                    httpContext[ContextKeyHttp.Request][ContextKeyHttpRequest.Headers]?.referer,
                },
              },
            })
          : "{}",
  ...
  protected slimDownLocally(object: any): any {
    ...
    if (
      ...
      object[ContextKey.CallerContext][ContextKeyCaller.Stack] = object[ContextKey.CallerContext][
        ContextKeyCaller.Stack
      ].slice(0, 2);
    }
  }
  ...
  private addLambdaKeys(level: Level, obj: BaseContext) {
    ...
    if (obj[ContextKey.Lambda]?.[ContextKeyLambda.RequestId])
      obj[ContextKey.LambdaRequestId] = obj[ContextKey.Lambda]?.[ContextKeyLambda.RequestId];
