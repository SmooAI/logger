...
export default class Logger {
  ...
  constructor(
    ...
    if (options.context) {
      ...
      if (
        ContextKey.CorrelationId in options.context &&
        options.context[ContextKey.CorrelationId]
      ) {
  ...
  protected setContextConfig(config: ContextConfig | undefined) {
    this.contextConfig = config
      ? config
      : global?.process.env.LOGGER_CONTEXT_CONFIG &&
          global?.process.env.LOGGER_CONTEXT_CONFIG in this._configSettings
    ...
    this.addBaseContext({
      [ContextKey.Http]: {
        [ContextKeyHttp.Request]: {
          ...
          [ContextKeyHttpRequest.UserAgent]: this.getHeaderValue(
            request.headers,
            ContextHeader.UserAgent,
          ),
    ...
    this.addBaseContext({
      [ContextKey.Http]: {
        [ContextKeyHttp.Response]: {
          ...
          [ContextKeyHttpResponse.Headers]: response.headers
            ? this.getHeadersObject(response.headers)
            : {},
      ...
      [ContextKey.Http]: {
        [ContextKeyHttp.Response]: {
          ...
          [ContextKeyHttpResponse.Headers]: context.headers
            ? this.getHeadersObject(context.headers)
            : {},
        },
      },
    });
  ...
  public getHttpRequestOriginDomain(): string | undefined {
    const originUrl =
      this.baseContextKey(ContextKey.Http)?.[ContextKeyHttp.Request]?.[
        ContextKeyHttpRequest.Headers
      ]?.["origin"] ||
      this.baseContextKey(ContextKey.Http)?.[ContextKeyHttp.Request]?.[
        ContextKeyHttpRequest.Headers
  ...
  private handleError(arg: any, object: any) {
    if (arg instanceof Error) {
      object[ContextKey.Error] = object[ContextKey.Error]
        ? `${object[ContextKey.Error]}; ${arg.message}`
        : arg.message;
      object[ContextKey.ErrorDetails] = [
        ...(object[ContextKey.ErrorDetails] ?? []),
        serializeError(arg),
      ];
    }
  ...
  protected buildLogObject(level: Level, args: any[]): any[] {
    ...
    for (const arg of args) {
      if (arg instanceof Error) {
        ...
        object[ContextKey.Message] = object[ContextKey.Message]
          ? `${object[ContextKey.Message]}; ${arg}`
          : arg;
      }
    }
  ...
  private prettyStringify(object: any): string {
    ...
    str = str.replace(
      /"msg": "(.*?)",\n/g,
      (_, msg) => `"msg": "${picocolors.bold(picocolors.green(msg))}",\n`,
    );
