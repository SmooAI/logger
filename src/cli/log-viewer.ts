...
if (!existsSync(binaryPath)) {
  console.error(
    `[smooai-log-viewer] No binary found for ${platform}/${arch}. Expected to find:`,
    `\n  ${binaryPath}`,
  );
  console.error(
    '[smooai-log-viewer] Ensure Rust is installed and run "pnpm run log-viewer:bundle" to build the desktop viewer.',
  );
  process.exit(1);
}
