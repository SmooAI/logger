...
async function tryDownload() {
    try {
        ...
        if (!response.ok) {
            console.log(`bundle-log-viewer: download returned ${response.status}, falling back to cargo build`);
            return false;
        }
        ...
        console.log(`bundle-log-viewer: download failed (${error.message}), falling back to cargo build`);
        return false;
    }
}
